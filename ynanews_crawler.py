...
def authenticate_with_oauth():
    ...
    try:
        ...
        # 토큰 파일에 저장
        token_path = os.getenv("EMAIL_OAUTH2_TOKEN", "token.json")
        with open(token_path, "w") as f:
            f.write(GMAIL_CREDS.to_json())
        logging.info(f"Gmail 인증 성공 (토큰 저장: {token_path})")
...
def check_gmail_credentials():
    ...
    if not os.path.exists(token_path):
        logging.info(f"토큰 파일 없음 ({token_path}), 인증 진행")
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            try:
                ...
                logging.info(f"토큰 갱신 완료 ({token_path})")
                return True
            except Exception as e:
...
def main():
    ...
    def fetch_and_filter(art, pbar):
        # 제목 매칭 확인
        if is_relevant_title(art.title):
            # 매칭된 경우에만 본문 가져오기
    ...
    with tqdm(
        ...
        for art in filtered:
            ...
            # 본문 길이 제한 (5000자)
            art["formatted_content"] = art["content"][:5000] + (
                "..." if len(art["content"]) > 5000 else ""
            )
    ...
    if article_count > 0:
        for idx, art in enumerate(filtered, start=1):
            ...
            # 임시 변수에 replace 작업 수행
            formatted_content = art["formatted_content"].replace("\n", "<br>")
